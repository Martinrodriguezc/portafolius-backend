...
const PROTOCOLS: ProtocolDef[] = [
  ...
  // Añadir aquí otros protocolos: E-FAST, TVP, Ocular, etc.
];
    ...
    for (const p of PROTOCOLS) {
      ...
      const protRes = await pool.query<{ id: number }>(
        `INSERT INTO protocol(key,name)
           ...
           WHERE NOT EXISTS (
             SELECT 1 FROM protocol WHERE key = CAST($1 AS VARCHAR(50))
           ) RETURNING id`,
        [p.key, p.name]
      );
      const protocolId =
        ...
        for (const it of sec.items) {
          await pool.query(
            `INSERT INTO protocol_item(section_id,key,label,score_scale,max_score)
               SELECT $1, CAST($2 AS VARCHAR(100)), CAST($3 AS VARCHAR(255)), CAST($4 AS VARCHAR(20)), $5
               WHERE NOT EXISTS (
